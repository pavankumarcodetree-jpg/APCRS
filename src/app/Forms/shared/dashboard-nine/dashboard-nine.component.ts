...
@Component({
    ...
    styleUrl: './dashboard-nine.component.css'
})
export class DashboardNineComponent {
    ...
    // ageDistributionChart(data: any) {
    //   const categories = ['0-6', '7-14', '15-59', '60+'];
    //   const values = [
    //     data.AGE_0_6,
    //     data.AGE_7_14,
    //     data.AGE_15_59,
    ...
    //   Highcharts.chart('age-distribution-chart', {
    //     chart: {
    //       type: 'column'
    //     },
    //     title: {
    ...
    //         return `Age Group: <b>${this.category}</b><br>Percentage: <b>${this.y}%</b>`;
    //       }
    //     },
    //     series: [{
    //       name: 'Age Group',
    ...
    //  // Set custom colors (you can adjust these)
    //   const colorPalette = ['#0072B2', '#F9E79F', '#58D68D', '#AF7AC5'];

    //   const pieData = categories.map((label, index) => ({
    //     name: label,
    //     y: values[index],
    //     color: colorPalette[index],
    //     sliced: index === 0,    // pull out first slice
    //     selected: index === 0   // highlight first slice
    //   }));

    //   // Column (bar) chart
    //   Highcharts.chart('age-bar-chart', {
    //     chart: {
    //       type: 'column'
    //     },
    //     title: {
    //       text: '', // Title handled in HTML
    //     },
    //     credits: {
    //       enabled: false
    //     },
    ...
    //   // Pie chart
    //   // Highcharts.chart('age-pie-chart', {
    //   //   chart: {
    //   //     type: 'pie'
    //   //   },
    //   //   title: {
    ...
    //       dataLabels: {
    //         enabled: true,
    //         format: '{point.name} years old: {point.y:.2f} %',
    ...
    //       ...item,
    //       sliced: i === 0,     // Pull out the first slice (optional)
    //       selected: i === 0    // Visually highlight the first slice
    //     }))
    //   }]
    // } as any);
    ...
    //         return `${this.point.name} years old: <b>${this.y.toFixed(2)}%</b>`;
    //       }
    //     },
    ...
    //       colorByPoint: false,
    //       data: [
    //         { name: '0-6', y: data.AGE_0_6, color: '#0072B2' },
    //         { name: '7-14', y: data.AGE_7_14, color: '#F9E79F' }, // Replace 0 with real data if available
    //         { name: '15-59', y: data.AGE_15_59, color: '#58D68D' },
    //         { name: '60+', y: data.AGE_GREAT_60, color: '#AF7AC5' }
    //       ]
    //     }]
    //   } as any);
    // }
    ...
    ageDistributionChart(data: any) {
        ...
        // --- Pie Chart (with combined 0-14 group) ---
        // Highcharts.chart('age-pie-chart', {
        //   chart: { type: 'pie' },
        //   title: { text: '' },
        //   credits: { enabled: false },
        ...
        Highcharts.chart('age-pie-chart', {
            ...
            tooltip: {
                formatter: function (this: any) {
                    ...
                    // Map the count based on the name of the age group
                    switch (this.point.name) {
                        case '0-6':
                            count = data.AGE_0_6_CNT;
                            break;
                        case '7-14':
                            count = data.AGE_7_14_CNT;
                            break;
                        case '15-59':
                            count = data.AGE_15_59_CNT;
                            break;
                        case '60+':
                            count = data.AGE_GREAT_60_CNT;
                            break;
                    }

                    return `${this.point.name} years old:<br/>
            ...
            series: [{
                ...
                data: [
                    { name: '0-6', y: data.AGE_0_6, color: '#0072B2' },
                    { name: '7-14', y: data.AGE_7_14, color: '#F9E79F' },
                    { name: '15-59', y: data.AGE_15_59, color: '#58D68D' },
                    { name: '60+', y: data.AGE_GREAT_60, color: '#AF7AC5' }
                ]
            }]
    ...
    async get_Data3(): Promise<void> {
        try {
            ...
            if (responce.code) {
                ...
                console.log(this.DataList4, "DataList3");
            } else {
                this.spinner.hide();
            }
        } catch (error) {
    ...
    async get_Data4(): Promise<void> {
        try {
            ...
            if (responce.code) {
                ...
                console.log(this.DashBoardList3, "DashBoardList4");
    ...
    async get_Data5(): Promise<void> {
        try {
            ...
            if (responce.code) {
                ...
                console.log(this.DashBoardList2, "DashBoardList5");
    ...
    Dash_Age_Group_List: any[] = [];
    async get_Age_Group_Data(): Promise<void> {
        try {
            // debugger;
            const req = new basemodel();
            req.type = "117";
            this.spinner.show();
            this.Dash_Age_Group_List = [];
            let responce: any = await this.auth.auth_utilities_rtgs(req);
            this.spinner.hide();
            if (responce.code) {
                this.Dash_Age_Group_List = responce.Details;
                console.log(this.Dash_Age_Group_List, "Dash_Age_Group_List");
                this.ageDistributionChart(this.Dash_Age_Group_List[0]);
    ...
    navigateToMetric(metricName: string) {
        this.router.navigate(['/shared/Tabs-TFR'], {
            queryParams: { metric: metricName }
        });
    }

    navigateToMetrictwo(metricName: string) {
        this.router.navigate(['/shared/Tabs-BRS'], {
            queryParams: { metric: metricName }
        });
    }

    navigateToCrsmethod(metricName: string) {
        this.router.navigate(['/shared/Tabs-BRS'], {
            queryParams: { metric: metricName }
        });
    ...
    navigateToRchmethod(metricName: string) {
        this.router.navigate(['/shared/Tabs-TFR'], {
            queryParams: { metric: metricName }
        });
        //console.log(metricName);
    }
    navigateToSec(metricName: string) {
        this.router.navigate(['/shared/Tabs-SEC'], {
            queryParams: { metric: metricName }
        });
        //console.log(metricName);
    }
