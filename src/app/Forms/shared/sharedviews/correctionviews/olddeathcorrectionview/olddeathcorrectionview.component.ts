...
@Component({
    selector: 'app-olddeathcorrectionview',
    templateUrl: './olddeathcorrectionview.component.html',
    styleUrl: './olddeathcorrectionview.component.css'
})
export class OlddeathcorrectionviewComponent {
    @Input() applicationid: any;
    @Input() applicationregnumber: any;
    @Input() typeofcorrection: any;
    @Input() olddeathview: any;
    UROLE: any;
    showcheckbox = false;
    contentuploadurl = '';
    contentshowurl = '';
        ...
        if (this.val.isEmpty(this.userremarks)) {
            ...
            try {
                try {
                    ...
                    this.spinner.show();
                    let responce: any = await this.auth.auth_pkgcrsolddeath_service(req);
                    this.spinner.hide();
    ...
    async inbox_Save(): Promise<void> {
        if (this.val.isEmpty(this.userseekclarificationremarks)) {
            ...
            try {
                ...
                req.json2 = this.draft_details_array;
                this.spinner.show(); 
                let responce: any = await this.auth.auth_pkgcrsoldbirth_service(req);
                this.spinner.hide();
