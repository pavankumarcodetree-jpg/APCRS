...
@Component({
    selector: 'app-nac-search-birth-event',
    templateUrl: './nac-search-birth-event.component.html',
    styleUrl: './nac-search-birth-event.component.css'
})
export class NacSearchBirthEventComponent {
    ...
    async get_birth_table(): Promise<void> {
        try {
            ...
            if (responce.code) {
                ...
                console.log(this.birth_depth_table_array);
                return;
            } else {
                this.spinner.hide();
                this.birth_depth_table_array = [];
    ...
    getbuttonenabled(agency: any) {
        ...
        this.applicationnumber = agency.BR_APPLICATION_ID;
        this.NAC_application_Id = agency.BR_NAC_APPLICATION_ID;
        this.NacSearchRDO = this.getBirthColumn('NAC_FLAG');
        console.log('NacSearchRDO', this.NacSearchRDO)
        this.radiobutton = true;
        if (agency.BR_APPLICATION_ID != null) {
            this.drafttype = 'BIRTHEDIT';
        }
    }

    getBirthColumn(columnName: string): boolean {
        return this.birth_depth_table_array.some(row => row[columnName] == "0" && row['BR_NAC_APPLICATION_ID'] === this.NAC_application_Id);
    }
    getRdo() {
        this.preview_tittle = "Rdo Birth Registration(జనన నమోదు)";
        this.viewmodel = "Rdo";
        this.typeofcorrection = "Rdo";

    }
    PathReportString: any; typeofcorrection: any; applicationid: any; preview_tittle = "";
    viewmodel = "nacsearchbirth"; drafttype = '';
