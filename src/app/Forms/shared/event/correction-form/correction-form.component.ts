...
@Component({
  ...
  styleUrl: './correction-form.component.css'
})
export class CorrectionFormComponent {
...
if (inputsource == 'documenent_descrption') {
  this.draft_details_array.documenent_descrption =
    ...
    fields: { name: string; disabled: boolean }[] = [
      { name: 'Childs Information', disabled: false },
      { name: 'Child Name Information', disabled: false },
      { name: 'Confirm Child Name', disabled: false },
      { name: 'Add Name Documents', disabled: false },
      { name: 'Child & Parents Information', disabled: false }
    ];
    ...
    onCheckboxChange(event: any, field: any) {
      if (event.target.checked) {
        ...
        const index = this.selectedFields.indexOf(field.name);
        if (index > -1) {
          this.selectedFields.splice(index, 1);
        }
      }
  
      // Disable all divs that are not selected
      this.fields.forEach(f => {
        f.disabled = !this.selectedFields.includes(f.name);
      });
    }
