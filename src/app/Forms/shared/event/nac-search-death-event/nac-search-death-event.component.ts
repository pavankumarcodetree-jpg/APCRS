...
@Component({
    selector: 'app-nac-search-death-event',
    templateUrl: './nac-search-death-event.component.html',
    styleUrl: './nac-search-death-event.component.css'
})
export class NacSearchDeathEventComponent {
    ...
    async get_birth_table(): Promise<void> {
        try {
            ...
            if (responce.code) {
                ...
                console.log('birth_depth_table_array', this.birth_depth_table_array)
    ...
    async Backwindow() {
        this.viewmodel = 'nacsearchdeath';
        this.radiobutton = false;
    }
    radiobutton: boolean = false; NAC_application_Id: any;
    NacSearchRDO: boolean = false;
    getbuttonenabled(agency: any) {
        
        this.applicationnumber = agency.DR_APPLICATION_ID;
        this.NAC_application_Id = agency.DR_NAC_APPLICATION_ID;
        this.NacSearchRDO = this.getBirthColumn('NAC_FLAG');
        this.radiobutton = true;
        if (agency.DR_APPLICATION_ID != null) {
            this.drafttype = 'DEATHEDIT';
        }
    }
    getBirthColumn(columnName: string): boolean {
        
        return this.birth_depth_table_array.some(row => row[columnName] == "0" && row['DR_NAC_APPLICATION_ID'] === this.NAC_application_Id);
    }
    getRdo() {
        this.preview_tittle = "Rdo Death Registration(మరణ నమోదు)";
        this.viewmodel = "Rdo";
        this.typeofcorrection = "Rdo";

    }
    PathReportString: any; typeofcorrection: any; applicationid: any; preview_tittle = "";
    viewmodel = "nacsearchdeath"; drafttype = '';
    async downloadorder(agency: any) {
        try {

            const req = new basemodel();
            req.type = '1022';
            req.param1 = agency.DR_NAC_APPLICATION_ID;
            req.param4 = 'deathnac';
            req.param5 = 'both';
            ...
            if (responce.code) {
                ...
                if (responce.url) {
                    ...
                    this.applicationnumber = agency.DR_NAC_APPLICATION_ID;
                }
