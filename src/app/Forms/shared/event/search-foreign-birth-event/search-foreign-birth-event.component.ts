...
@Component({
  selector: 'app-search-foreign-birth-event',
  templateUrl: './search-foreign-birth-event.component.html',
  styleUrl: './search-foreign-birth-event.component.css'
})
export class SearchForeignBirthEventComponent {
  ...
  async get_birth_table(): Promise<void> {
    ...
    try {
      if (this.validationforinputs()) {
        ...
        // req.rucode=this.RuCode;
        let responce: any = await this.auth.auth_pkgcrsforeignbirth_service(req);
