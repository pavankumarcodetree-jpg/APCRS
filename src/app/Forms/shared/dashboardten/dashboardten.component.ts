...
import { Observable, Observer, share, Subject } from 'rxjs';
...
@Component({
    selector: 'app-dashboardten',
    templateUrl: './dashboardten.component.html',
    styleUrl: './dashboardten.component.css'
})
export class DashboardtenComponent {
...
goToDistrict(district: any) { 
    ...
    constructor(private spinner: NgxSpinnerService,
        ...
        setTimeout(() => {
            // Update value
            this.observable.next(true);           // Trigger odometer
        }, 2000);
        ...
        //Age Structure
        this.chartOptions10 = {
            series: [4.9, 9.3, 66.5, 16.1],
            chart: {
                type: "donut",
                height: 400,
            },
            labels: ["0-6 years Old : 4.98 %", "7-14 years old : 9.38 %", "15-59 years old: 66.51 %", "60+ years old: 16.13 %"],
            responsive: [
                {
                    breakpoint: 480,
                    options: {
    ...
    ngOnInit(): void {
        ...
        try {
            if (sessionStorage.getItem('_Uenc') !== '') {
                ...
                if (obj != '' && obj != undefined && obj != null) {
                    ...
                    // Then call every 30 seconds (30000 ms)
                    // this.intervalId = setInterval(() => {
                    //   debugger;
                    //   console.log('Calling get_Data3() at:', new Date().toLocaleTimeString());
                    //   this.dataChange2();
                    // }, 30000);
                    ...
                    if (this.objDashBoard.YearValue == 2025) {
                        this.intervalId = setInterval(() => {
                            //  console.log('Calling:', new Date().toLocaleTimeString());
                            this.dataChange2();
                        }, 310000); // 5 min 10 sec = 310000 ms
                    }



                    this.loopDigits('digit1');
                    this.loopDigits('digit2');
                    this.loopDigits('digit3');
                    this.loopDigits('digit4');
                    this.loopDigits('digit5');
                    this.loopDigits('digit6');
                    this.loopDigits('digit7');
                    this.loopDigits('digit8');
                    this.animateCount('todayCount', 123, 15000);
                    this.animateCount('yearCount', 2435, 15000);


                    // Tile Count
                    this.animateCount('BirthtodayCount', 0, 15000);
                    this.animateCount('BirthyearCount', 230171, 15000);
                    this.animateCount('DeathtodayCount', 0, 15000);
                    this.animateCount('DeathyearCount', 188197, 15000);
                    this.animateCount('TFRtodayCount', 0, 15000);
                    this.animateCount('TFRyearCount', 0.36, 15000);
                    this.animateCount('SexRatiotodayCount', 0, 15000);
                    this.animateCount('SexRatioyearCount', 925.55, 15000);
                    this.animateCount('NMRtodayCount', 0, 15000);
                    this.animateCount('NMRyearCount', 0, 15000);
                    //
        ...
        const date = now.toLocaleDateString('en-GB'); // gives DD/MM/YYYY format

        this.lastUpdated = `${time} ${date}`;
    ...
    async get_total_birth_death(): Promise<void> {
        try {
            ...
            if (responce.code) {
                ...
                // console.log(this.total_birth_death_array, "total_birth_death_array");
            } else {
                this.total_birth_death_array = [];
                this.spinner.hide();
            }
    ...
    async get_all_state_summary_birth_death(): Promise<void> {
        try {
            ...
            if (responce.code) {
                ...
                // console.log(this.all_state_summary_birth_death_array, "all_state_summary_birth_death_array");
            } else {
                this.all_state_summary_birth_death_array = [];
                this.spinner.hide();
            }
    ...
    async get_world_summary_birth_death(): Promise<void> {
        try {
            ...
            if (responce.code) {
                ...
                // console.log(this.world_summary_birth_death_array, "world_summary_birth_death_array");
            } else {
                this.world_summary_birth_death_array = [];
                this.spinner.hide();
            }
    ...
    async get_summary_gender(): Promise<void> {
        try {
            ...
            if (responce.code) {
                ...
                // console.log(this.summary_gender_array, 'summaty')
    ...
    async get_summary_treads(): Promise<void> {
        try {
            ...
            if (responce.code) {
                ...
                //  console.log(this.summary_treads_array, "summary_treads_array");
            } else {
                this.summary_treads_array = [];
                this.spinner.hide();
            }
    ...
    summary_Age_array: any[] = [];
    async get_Age_treads(): Promise<void> {
        try {
            const req = new basemodel();
            req.type = '3011'//'3004';
            this.spinner.show();
            this.summary_Age_array = [];
            let responce: any = await this.auth.auth_utilities_service02(req);
            this.spinner.hide();
            if (responce.code) {
                this.summary_Age_array = responce.Details;
                // console.log(this.summary_Age_array, "summary_Age_array");
            } else {
                this.summary_Age_array = [];
                this.spinner.hide();
            }
    ...
    async get_birth_death_treads(): Promise<void> {
        try {
            ...
            if (responce.code) {
                ...
                //  console.log(this.birth_death_treads_array, "birth_death_treads_array");
    ...
    async get_age_group_treads(): Promise<void> {
        try {
            ...
            if (responce.code) {
                ...
                //console.log(this.birth_age_group_array, "birth_age_group_array");
    ...
    async get_age_group_percentage(): Promise<void> {
        try {
            ...
            if (responce.code) {
                ...
                // console.log(this.birth_age_percentage_array, "birth_age_percentage_array");
    ...
    async get_death_distribution(): Promise<void> {
        try {
            ...
            if (responce.code) {
                ...
                //console.log(this.death_distribution_array, "death_distribution_array");
    ...
    // populationPredictionChart(data: any) {
    //   const years: any[] = [];
    //   const predictions: any[] = [];

    //   data.forEach((item: any) => {
    //     years.push(item.POP_YEAR);
    //     predictions.push(item.POP_PREDIC);
    //   });

    //   Highcharts.chart('population-prediction-chart', {
    //     chart: {
    //       type: 'column'
    //     },
    //     title: {
    //       text: 'Population Prediction by Year',
    //       align: 'left',
    //       style: {
    //         color: '#3f4141'
    //       }
    ...
    populationPredictionChart(data: any) {
        ...
        Highcharts.chart('population-Projection-chart', {
            chart: {
                type: 'column'
            },
            title: {
                text: 'Population Projection  by Year',
                align: 'left',
                style: {
                    color: '#3f4141'
                }
            ...
            tooltip: {
                formatter: function (this: Highcharts.Point) {  // or Highcharts.PointLabelObject
                    const num = Number(this.y);
                    return `Population: <b>${num.toLocaleString('en-IN')}</b>`;
                }
            },
            series: [{
                name: 'Population',
    ...
    age_group_gender_Chart(obj: any) {
        ...
        Highcharts.chart('age-gender-chart', {
            ...
            yAxis: {
                labels: { style: { color: '#3f4141' } },
                title: { text: 'Number of Births', style: { color: '#3f4141' } },
            },
            series: [
                {
                    name: 'Male Births',
    ...
    onHover(district: any) {
        ...
        this.totalbirthcount = obj.TOTAL_BIRTH;
        this.male_count = obj.MALE_BIRTH;
        this.female_count = obj.FEMALE_BIRTH;
        this.selectedDistrict = district;
    }

    onLeave() {
        this.totalbirthcount = this.getsum_of_coloumn('TOTAL_BIRTH');
        this.male_count = this.getsum_of_coloumn('MALE_BIRTH');
        this.female_count = this.getsum_of_coloumn('FEMALE_BIRTH');
        this.selectedDistrict = null;
    }


    loopDigits(varName: string) {
        let value = 0;
        setInterval(() => {
            (this as any)[varName] = value;
            value = (value + 1) % 10; // Loop from 0 to 9
        }, 1000); // 1 second delay between changes
    }


    animateCount(variableName: string, target: number, duration: number) {
        const steps = 50;
        ...
        const interval = setInterval(() => {
            ...
            (this as any)[variableName] = isFloat
                ? parseFloat(current.toFixed(2))  // round to 2 decimal places
                : Math.floor(current);
        }, intervalTime);
    }
    ...
    objDashBoard = {
        SummaryYearValue: null as any,
        PopCount: null as any,
        YearValue: 2026,
        selectedDuration: 10
    };
    durations: number[] = [10, 20, 30, 40, 50];
    yearList: number[] = [];
    DashBoardList: any[] = [];
    DashBoardList1: any[] = [];
    DashBoardList2: any[] = [];
    ...
    async get_Data2(): Promise<void> {
        try {
            const req = new basemodel();
            req.type = "101";
            req.param1 = this.objDashBoard.YearValue;
            this.spinner.show();
            this.DashBoardList1 = [];
            let responce: any = await this.auth.auth_utilities_rtgs(req);
            this.spinner.hide();
            if (responce.code) {
                ...
                console.log(this.DashBoardList1, 'DashBoardList1');
            } else {
                this.DashBoardList1 = [];
                this.spinner.hide();
            }
    ...
    async get_Data3(): Promise<void> {
        ...
        try {
            ...
            this.spinner.hide();
            if (responce.code) {
                this.DataList4 = responce.Details;
                console.log(this.DataList4, 'DataList4');
    ...
    async get_Data4(): Promise<void> {
        try {
            const req = new basemodel();
            req.type = "104";
            req.param1 = this.objDashBoard.selectedDuration;
            this.spinner.show();
            this.DashBoardList3 = [];
            let responce: any = await this.auth.auth_utilities_rtgs(req);
            this.spinner.hide();
            if (responce.code) {
                this.DashBoardList3 = responce.Details;
                console.log(this.DashBoardList3, 'DashBoardList3');

                this.populationPredictionChart(this.DashBoardList3);
            } else {
                this.DashBoardList3 = [];
                this.spinner.hide();
            }
    ...
    async get_Data5(): Promise<void> {
        try {
            ...
            this.spinner.hide();
            if (responce.code) {
                this.DashBoardList2 = responce.Details;
    ...
    async get_summary_birth_death(): Promise<void> {
        try {
            ...
            if (responce.code) {
                ...
                console.log(this.summary_birth_death_array, 'summary_birth_death_array');
                this.totalbirthcount = this.getsum_of_coloumn('TOTAL_BIRTH');
                this.male_count = this.getsum_of_coloumn('MALE_BIRTH');
                this.female_count = this.getsum_of_coloumn('FEMALE_BIRTH');
                this.selectedDistrict = null;
    ...
    async dataChange() {
        debugger;
        const year = this.objDashBoard.YearValue;
        const validYears = [2023, 2024, 2025, 2026, 2027, 2028, 2029, 2030, 2031, 2032, 2033, 2034, 2035, 2036];

        if (validYears.includes(year)) {
            await this.get_Data3();
            await this.get_Data();
            await this.get_Data2();
            await this.get_Data4();
            await this.get_summary_birth_death();
            this.objDashBoard.PopCount = "";
            this.objDashBoard.PopCount = this.DataList4[0].TOT_POP;
            this.objDashBoard.SummaryYearValue = year;
        } else {
            await this.get_Data5();
            this.objDashBoard.PopCount = "";

            const match = this.DashBoardList2.find(item => item.POP_YEAR === year);
            if (match) {
                this.objDashBoard.PopCount = match.POP_PREDIC;
            }
        }
    }
    ...
    async dataChange2() {
        ...
        if (this.DataList4?.length) {
            this.animateNumber(this.objDashBoard.PopCount, this.DataList4[0].TOT_POP, 30000);
        }
        // this.animateNumber(this.objDashBoard.PopCount, this.DataList4[0].TOT_POP, 30000);
        // } else {
        //  await this.get_Data5();
        ...
        // const match = this.DashBoardList2.find(item => item.POP_YEAR === year);
        // if (match) {
        // this.animateNumber(this.objDashBoard.PopCount, match.POP_PREDIC, 30000);
        // }
        //}
    }
    onDurationChange(event: any) {
        this.objDashBoard.selectedDuration = +event.target.value;
        this.get_Data4();
    }
    populateYearList(): void {
    ...
    animateNumber(current: number, target: number, duration: number = 30000) {
        ...
        const timer = setInterval(() => {
            current += increment;
            this.objDashBoard.PopCount = current; // format as Indian numbering system
            if (current === target) {
                clearInterval(timer);
            }
        }, stepTime);
    ...
    //   const increment = range > 0 ? 1 : -1; // direction
    //   const stepTime = Math.abs(Math.floor(duration / range));

    //   let currentNumber = start;
    //   const timer = setInterval(() => {
    //     currentNumber += increment;
    //     this.objDashBoard.PopCount = currentNumber;
    ...
    //   animateNumberDigitByDigit(current: number, target: number, delay: number = 50) {
    ...
    //   let index = currentDigits.length - 1; // start from last digit

    //   const animateDigit = () => {
    //     if (index < 0) return; // stop when all digits updated

    //     if (currentDigits[index] !== targetDigits[index]) {
    //       currentDigits[index] = (parseInt(currentDigits[index]) + 1).toString();
    //       if (parseInt(currentDigits[index]) > 9) currentDigits[index] = '0';

    //       const newNumber = parseInt(currentDigits.join(''), 10);
    //       this.objDashBoard.PopCount = newNumber;
    //       //toLocaleString('en-IN');
    //       setTimeout(animateDigit, delay); // keep animating same digit until it matches
    //     } else {
    //       index--; // move to next digit
    //       setTimeout(animateDigit, delay);
    //     }
    //   };
    ...
    // this code is dashboardnine start


    //this code is dashboardnine end


     chartOptions11: Highcharts.Options = {
    chart: {
      type: 'pie',
      height: 400
    },
    ...
    title: {
      text: 'Pie chart - Age Wise Structure',
      align: 'left',
    style: {
     color: '#3f4141'
    }
    ...
    series: [
      {
        ...
        data: [
          {
            name: '0-6 years Old : 4.98 %',
            y: 4.9
          },
          {
            name: '7-14 years old : 9.38 %',
            y: 9.3
          },
          {
            name: '15-59 years old : 66.51 %',
            y: 66.5
          },
          {
            name: '60+ years old : 16.13 %',
            y: 16.1
          }
        ]
      }
