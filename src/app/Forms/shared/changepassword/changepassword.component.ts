...
export class ChangepasswordComponent {
    ...
    is_send_otp_disable: boolean = false; is_resend_otp: boolean = false; is_mobile_disable: boolean = false;
  ...
  async ngOnInit(): Promise<void> {
    try {
        ...
        this.Changpassform.username = obj[0].UNAME;
        this.Get_captcha();
        }
  ...
  async Get_captcha(): Promise<void> {
    try {
      ...
      req.param2 = sessionStorage.getItem('appid');
      req.param3 = sessionStorage.getItem('_cltkn');
  ...
  async submit_form(): Promise<void> {
    ...
    @Output() changepasswordshow = new EventEmitter();
     
    childClick(){
      this.changepasswordshow.emit(true);
    }
}
