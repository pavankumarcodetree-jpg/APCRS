...
@Component({
    selector: 'app-stillbirthhome',
    templateUrl: './stillbirthhome.component.html',
    styleUrl: './stillbirthhome.component.css'
})
export class StillbirthhomeComponent {
    contentuploadurl = '';
    contentshowurl = '';
    ...
    draft_details_array = {
        ...
        legal: {
            ...
            causeoffoetaldeath: '',
            causeoffoetaldeath_VAL: '',
        },
    };
    ngAfterViewInit(): void {
        // Initialize Fancybox for images and PDFs
        Fancybox.bind("[data-fancybox]", {
            Toolbar: {
                display: ["zoom", "fullscreen", "close"],
            },
            iframe: {
                preload: false, // Prevent unnecessary preloading
            }
        });
    }
    RU_CODE: any; UROLE: any; obj: any
            ...
            if (this.photoselectedFiles.length != 0) {
                ...
                if (this.photoselectedFiles.length > 0) {
                    ...
                    for (let ph = 0; ph < maxlen; ph++) {
                        ...
                        phform.append('input01', 'Stillbith');
                        phform.append('input02', 'stillbirthregistration');
    ...
    multiphotofilepath: any; fileSelected: boolean = false;
    ...
    async documentdelte(obj: any): Promise<void> {
        try {
            ...
            const req = new basemodel();
            req.type = '1014';
            req.param1 = obj.SR_APPLICATION_ID;
            req.param2 = obj.SR_DOCUMENT_CODE;
            this.spinner.show();
            let rsdata: any = await this.auth.auth_pkgcrsstillbirth_service(req);
            this.spinner.hide();
            if (rsdata.code) {
    ...
    async get_SupportingDocuments(): Promise<void> {
        try {
            ...
            let responce: any = await this.auth.auth_utilities_service(req);
            console.log('still birth : ' + responce);
            this.spinner.hide();
            if (responce.code) {
    ...
    async PlaceofBirth_urbanchange() {
    ...
    async staticstical_urbanchange() {
    ...
    legal_checkAndShowAlert() {
        ...
        if (this.mother_check_status && this.father_check_status && this.father_mother_check_status && this.mother_Aadharno_status && this.father_Aadharno_status) {
    ...
    Ageofthemothercheck() {
        ...
        if (parseInt(this.draft_details_array.legal.age_of_the_mother) < 18) {
            ...
            this.alt.toasterror('Age of the mother (in completed years) at the time of this birth should be above 18 or more');
        }
    ...
    // async districtchangeser(type: any) {  
    //     console.log(type);
    //     if (type == 'placeofbirth') {  
    //         this.draft_details_array.placeofbirth.place_of_birth_rural_urabn = '';
    //         this.PlaceofBirth_urbanchange();
