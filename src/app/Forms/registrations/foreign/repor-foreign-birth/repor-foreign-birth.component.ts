...
@Component({
  selector: 'app-repor-foreign-birth',
  templateUrl: './repor-foreign-birth.component.html',
  styleUrl: './repor-foreign-birth.component.css'
})
export class ReporForeignBirthComponent {
  ...
  draft_details_array = {
    ...
    placeofbirth: {
      ...
      placeofbirth_value: '',
      place_of_birth_addressof: 'outside',
      address_place_of_birth_country: '',
      addressplace_of_birthoutsideindia: '',
      address_place_of_birth_country_address_t: '',
      dateofarrivaltoindia: '',
      address_Settlement_sameasaddress: '',
    },
    information: {
      Informant_fullname: '',
  ...
  async getdraft(): Promise<void> {
    try {
      ...
      req.type = '1000';
      req.param1 = 'FOREIGNBIRTH';
      this.spinner.show();
      let rsdata: any = await this.auth.auth_pkgcrsforeignbirth_service(req);
      ...
      if (this.photoselectedFiles.length != 0) {
        ...
        if (this.photoselectedFiles.length > 0) {
          ...
          for (let ph = 0; ph < maxlen; ph++) {
            ...
            phform.append('input01', 'Foreign');
            phform.append('input02', 'foreignregistration');
    ...
    if (this.multiphotoselectedFiles.length == 0) {
      ...
      if (this.multiphotoselectedFiles.length != 0) {
        ...
        if (this.multiphotoselectedFiles.length > 0) {
          ...
          this.multifileError = !this.multifileSelected;
          for (let ph = 0; ph < maxlen; ph++) {
            this.photofilepath = '';
            let filename = this.brapplicationid + '_' + this.RU_CODE;
            const phform = new FormData();
    ...
    this.pscall.google_translate(inputkeyval).subscribe(
      (response: any) => {
        ...
        if (response[0] != '') // if (response[0] == 'SUCCESS')
        {
          ...
          if (inputsource == 'addressplace_of_birthoutsideindia') {
            this.draft_details_array.placeofbirth.address_place_of_birth_country_address_t = this.suggestions[0];
          }
          if (inputsource == 'address_place_of_birth_country_address_tel') {
            this.draft_details_array.placeofbirth.address_place_of_birth_country_address_t = this.suggestions[0];
          }
        } else {
          if (inputsource == 'child_fullname') {
          ...
          if (inputsource == 'addressplace_of_birthoutsideindia') {
            this.draft_details_array.placeofbirth.addressplace_of_birthoutsideindia = '';
          }
          if (inputsource == 'address_place_of_birth_country_address_tel') {
            this.draft_details_array.placeofbirth.address_place_of_birth_country_address_t = '';
          }
        }
      },
      (error) => {
  ...
  onDateChange(event: any, type: any) {
    ...
    if (type == 'DateofArrivaltoIndia') {
      this.draft_details_array.placeofbirth.dateofarrivaltoindia = formattedDate || '';
  ...
  checkarrivalDate(selectedDate: any) {
    
    const currentDate = new Date();
    const selected = new Date(selectedDate);

    // Calculate the difference in time (in milliseconds)
    const timeDiff = currentDate.getTime() - selected.getTime();
    ...
    if (dayDiff > 60) {
      this.alt.toasterror('The date of arrival to India is more than 60 days request should be processed as per guidelines.');
    } else {
      // alert('The date is within 60 days.');
    }
  }
  ...
  @ViewChild('dateofarrivaltoindia') dateofarrivaltoindia!: NgModel;
  ...
  legal_from_check_inpts() {
    ...
    if (this.dateofarrivaltoindia) {
      this.dateofarrivaltoindia.control.markAsTouched();
      this.dateofarrivaltoindia.control.updateValueAndValidity();
      if (this.dateofarrivaltoindia.invalid) isInvalid = true;
    }
    // if (
    //   this.Place_of_birth_village &&
    //   this.draft_details_array.placeofbirth.place_of_birth != '1'
  ...
  legal_checkAndShowAlert() {
    ...
    if (this.mother_check_status && this.father_check_status && this.father_mother_check_status && this.child_name_status && this.child_Aadharno_status && this.father_Aadharno_status && this.mother_Aadharno_status) {
  ...
  async backwindowdraftbinding(obj: any) {
    ...
    this.draft_details_array.placeofbirth = this.draft_details_array.placeofbirth || {};
    this.draft_details_array.childinformation = this.draft_details_array.childinformation || {};
    this.draft_details_array.language = this.draft_details_array.language || {};
    this.draft_details_array.information = this.draft_details_array.information || {};

    this.draft_details_array.placeofbirth.place_of_birth_addressof = 'outside';
  ...
  //   foreignbirthinfodetails() {
  //     if (this.draft_details_array.placeofbirth.place_of_birth == "1") {
  //         this.draft_details_array.information = {
  //             Informant_fullname: this.obj[0].UDPNAME,
  //             Informant_fullname_t: '',
  //             Informant_fullname_val: '',
  //             Informant_email_id: '',
  //             Informant_mobile_no: this.obj[0].UMOBILE,
  //             Informant_aadhhar_no_mask: '',
  //             Informant_aadhhar_no: '',
  //             Informant_address: this.obj[0].RU_ADDRESS,
  //             Informant_address_t: '',
  //             Informant_address_val: '',
  //             information_value: '',
  //         }
  ...
  async PlaceofBirthchange() {
    ...
    this.dateofreportdesiable = false;
    if (this.UROLE == '105' || this.UROLE == '112' || this.UROLE == '111' || this.UROLE == '1007' || this.UROLE == '108') {
      this.dateofreportdesiable = true;
      console.log(this.draft_details_array.placeofbirth.place_of_birth);
      this.draft_details_array.placeofbirth.place_of_birth_state = this.obj[0].STATE_CODE;
  ...
  settlementradioenable = true;

  SameasAddressoftheParents(event: any) {
    

    this.settlementradioenable = false;
    this.draft_details_array.Permanent_addressof_parents.address_parent_sameasaddress = '';
    const selectedValue = event.target.value;
    this.draft_details_array.Permanent_addressof_parents.address_parent_sameasaddress = selectedValue;

    if (this.draft_details_array.Permanent_addressof_parents.address_parent_sameasaddress == 'Yes') {

      this.draft_details_array.Permanent_addressof_parents = {
        address_parent_sameasaddress: selectedValue,
        address_parent_addressof: this.draft_details_array.addressof_parents.addressof,
        address_parent_permenent_country: this.draft_details_array.addressof_parents.address_parent_country,
        address_parent_permenent_country_val: this.draft_details_array.addressof_parents.address_parent_country_address_val,
        address_parent_permenent_country_address: this.draft_details_array.addressof_parents.address_parent_country_address,
        address_parent_permenent_country_address_t: this.draft_details_array.addressof_parents.address_parent_country_address_t,
        address_parent_permenent_country_address_val: this.draft_details_array.addressof_parents.address_parent_country_address_val,
        address_parent_permenent_state: this.draft_details_array.addressof_parents.address_parent_state,
        address_parent_permenent_state_val: this.draft_details_array.addressof_parents.address_parent_state_val,
        address_parent_permenent_district: this.draft_details_array.addressof_parents.address_parent_district,
        address_parent_permenent_district_val: this.draft_details_array.addressof_parents.address_parent_district_val,
        address_parent_permenent_mmc: this.draft_details_array.addressof_parents.address_parent_mmc,
        address_parent_permenent_mmc_val: this.draft_details_array.addressof_parents.address_parent_mmc_val,
        address_parent_permenent_rural_urabn: this.draft_details_array.addressof_parents.address_parent_rural_urabn,
        address_parent_permenent_rural_urabn_val: this.draft_details_array.addressof_parents.address_parent_rural_urabn_val,
        address_parent_permenent_village: this.draft_details_array.addressof_parents.address_parent_Village_Town,
        address_parent_permenent_village_val: this.draft_details_array.addressof_parents.address_parent_Village_Town_val,
        address_parent_permenent_pincode: this.draft_details_array.addressof_parents.address_parent_pincode,
        address_parent_permenent_building_no: this.draft_details_array.addressof_parents.address_parent_building_name,
        address_parent_permenent_building_no_t: this.draft_details_array.addressof_parents.address_parent_building_name_t,
        address_parent_permenent_building_no_val: this.draft_details_array.addressof_parents.address_parent_building_name_val,
        address_parent_permenent_hose_no: this.draft_details_array.addressof_parents.address_parent_house_no,
        address_parent_permenent_hose_no_t: this.draft_details_array.addressof_parents.address_parent_house_no_t,
        address_parent_permenent_hose_no_val: this.draft_details_array.addressof_parents.address_parent_house_no_val,
        address_parent_permenent_street_name: this.draft_details_array.addressof_parents.address_parent_street_name,
        address_parent_permenent_street_name_t: this.draft_details_array.addressof_parents.address_parent_street_name_t,
        address_parent_permenent_street_name_val: this.draft_details_array.addressof_parents.address_parent_street_name_val,
        address_parent_permenent_locality: this.draft_details_array.addressof_parents.address_parent_locality,
        address_parent_permenent_locality_t: this.draft_details_array.addressof_parents.address_parent_locality_t,
        address_parent_permenent_locality_post_office_val: '',
        address_permenent_postoff: this.draft_details_array.addressof_parents.address_parent_postoff,
        address_permenent_postoff_val: this.draft_details_array.addressof_parents.address_parent_postoff_val,
      };
    }
    if (this.draft_details_array.Permanent_addressof_parents.address_parent_sameasaddress == 'No') {
      this.draft_details_array.Permanent_addressof_parents = {

        address_parent_sameasaddress: selectedValue,
        address_parent_addressof: 'inside',
        address_parent_permenent_country: '',
        address_parent_permenent_country_val: '',
        address_parent_permenent_country_address: '',
        address_parent_permenent_country_address_t: '',
      ...
      this.clearsameasSettlementaddress();
  ...
  sameasaddress_outofindia(event: any) {
    const selectedValue = event.target.value;
    this.draft_details_array.Permanent_addressof_parents = {
      address_parent_sameasaddress: this.draft_details_array.Permanent_addressof_parents.address_parent_sameasaddress,
      address_parent_addressof: selectedValue,
      address_parent_permenent_country: '',      //this.draft_details_array.Permanent_addressof_parents.address_parent_sameasaddress,
      address_parent_permenent_country_val: '',
      address_parent_permenent_country_address: '',
      address_parent_permenent_country_address_t: '',
      address_parent_permenent_country_address_val: '',
  ...
  sameasaddressSettlement_toggleRadio(value: string): void {
    
    this.loadalldropdowns();
    if (
      this.draft_details_array.placeofbirth.address_Settlement_sameasaddress === value
    ) {
      this.draft_details_array.placeofbirth.address_Settlement_sameasaddress =
        '';
      this.clearsameasSettlementaddress();
    } else {
      // this.draft_details_array.Permanent_addressof_parents.address_parent_sameasaddress =
      ...
      if (value == 'Yes') {
        ...
        this.draft_details_array.placeofbirth.place_of_birth = '',
          ...
          this.draft_details_array.placeofbirth.place_of_birth_state = this.draft_details_array.Permanent_addressof_parents.address_parent_permenent_state,
          this.draft_details_array.placeofbirth.place_of_birth_state_val = this.draft_details_array.Permanent_addressof_parents.address_parent_permenent_state_val,
          this.draft_details_array.placeofbirth.place_of_birth_district = this.draft_details_array.Permanent_addressof_parents.address_parent_permenent_district,
          this.draft_details_array.placeofbirth.place_of_birth_district_val = this.draft_details_array.Permanent_addressof_parents.address_parent_permenent_district_val,
          this.draft_details_array.placeofbirth.place_of_birth_rural_urabn = this.draft_details_array.Permanent_addressof_parents.address_parent_permenent_rural_urabn,
          this.draft_details_array.placeofbirth.place_of_birth_rural_urabn_val = this.draft_details_array.Permanent_addressof_parents.address_parent_permenent_rural_urabn_val,
          this.draft_details_array.placeofbirth.place_of_birth_mmc = this.draft_details_array.Permanent_addressof_parents.address_parent_permenent_mmc,
          this.draft_details_array.placeofbirth.place_of_birth_mmc_val = this.draft_details_array.Permanent_addressof_parents.address_parent_permenent_mmc_val,
          this.draft_details_array.placeofbirth.place_of_birth_village = this.draft_details_array.Permanent_addressof_parents.address_parent_permenent_village,
          this.draft_details_array.placeofbirth.place_of_birth_village_val = this.draft_details_array.Permanent_addressof_parents.address_parent_permenent_village_val,
          this.draft_details_array.placeofbirth.place_of_birth_pin_code = this.draft_details_array.Permanent_addressof_parents.address_parent_permenent_pincode,
          this.draft_details_array.placeofbirth.place_of_birth_building_no = this.draft_details_array.Permanent_addressof_parents.address_parent_permenent_building_no,
          this.draft_details_array.placeofbirth.place_of_birth_building_no_t = this.draft_details_array.Permanent_addressof_parents.address_parent_permenent_building_no_t,
          this.draft_details_array.placeofbirth.place_of_birth_building_no_val = this.draft_details_array.Permanent_addressof_parents.address_parent_permenent_building_no_val,
          this.draft_details_array.placeofbirth.place_of_birth_house_no = this.draft_details_array.Permanent_addressof_parents.address_parent_permenent_hose_no,
          this.draft_details_array.placeofbirth.Place_of_birth_house_no_t = this.draft_details_array.Permanent_addressof_parents.address_parent_permenent_hose_no_t,
          this.draft_details_array.placeofbirth.place_of_birth_house_no_val = this.draft_details_array.Permanent_addressof_parents.address_parent_permenent_hose_no_val,
          this.draft_details_array.placeofbirth.place_of_birth_street_name = this.draft_details_array.Permanent_addressof_parents.address_parent_permenent_street_name,
          this.draft_details_array.placeofbirth.place_of_birth_street_name_t = this.draft_details_array.Permanent_addressof_parents.address_parent_permenent_street_name_t,
          this.draft_details_array.placeofbirth.place_of_birth_street_name_val = this.draft_details_array.Permanent_addressof_parents.address_parent_permenent_street_name_val,
          this.draft_details_array.placeofbirth.Place_of_birth_locality = this.draft_details_array.Permanent_addressof_parents.address_parent_permenent_locality,
          this.draft_details_array.placeofbirth.Place_of_birth_locality_t = this.draft_details_array.Permanent_addressof_parents.address_parent_permenent_locality_t,
          this.draft_details_array.placeofbirth.Place_of_birth_locality_val = '',
          this.draft_details_array.placeofbirth.Place_of_birth_postoff = this.draft_details_array.Permanent_addressof_parents.address_permenent_postoff,
          this.draft_details_array.placeofbirth.Place_of_birth_postoff_val = '',
          this.draft_details_array.placeofbirth.placeofbirth_value = '',
          this.draft_details_array.placeofbirth.place_of_birth_addressof = 'outside',
          // this.draft_details_array.placeofbirth.address_place_of_birth_country= '',
          // this.draft_details_array.placeofbirth.addressplace_of_birthoutsideindia= '',
          // this.draft_details_array.placeofbirth.address_place_of_birth_country_address_t= '',
          // this.draft_details_array.placeofbirth.dateofarrivaltoindia= '',
          this.draft_details_array.placeofbirth.address_Settlement_sameasaddress = value

        if (this.draft_details_array.placeofbirth.place_of_birth_pin_code != "")
          this.Postalareas('placeofbirth');
      }
    }
  }
  clearsameasSettlementaddress() {
    ...
    this.draft_details_array.placeofbirth.place_of_birth = '',
      ...
      this.draft_details_array.placeofbirth.place_of_birth_pin_code = '',
      this.draft_details_array.placeofbirth.place_of_birth_building_no = '',
      this.draft_details_array.placeofbirth.place_of_birth_building_no_t = '',
      this.draft_details_array.placeofbirth.place_of_birth_building_no_val = '',
      this.draft_details_array.placeofbirth.place_of_birth_house_no = '',
      this.draft_details_array.placeofbirth.Place_of_birth_house_no_t = '',
      this.draft_details_array.placeofbirth.place_of_birth_house_no_val = '',
      this.draft_details_array.placeofbirth.place_of_birth_street_name = '',
      this.draft_details_array.placeofbirth.place_of_birth_street_name_t = '',
      this.draft_details_array.placeofbirth.place_of_birth_street_name_val = '',
      this.draft_details_array.placeofbirth.Place_of_birth_locality = '',
      this.draft_details_array.placeofbirth.Place_of_birth_locality_t = '',
      this.draft_details_array.placeofbirth.Place_of_birth_locality_val = '',
      this.draft_details_array.placeofbirth.Place_of_birth_postoff = '',
      this.draft_details_array.placeofbirth.Place_of_birth_postoff_val = '',
  ...
  copyadresscheck(event: Event) {
    ...
    if (isChecked) {
      ...
      this.draft_details_array.legal.mother_residence_state = this.draft_details_array.placeofbirth.place_of_birth_state;
      this.getdistrict('staticstical');
      this.draft_details_array.legal.mother_residence_district = this.draft_details_array.placeofbirth.place_of_birth_district;
      this.draft_details_array.legal.mother_residence_rural_urban = this.draft_details_array.placeofbirth.place_of_birth_rural_urabn;
      this.MandalMuncipality('staticstical');
      this.draft_details_array.legal.mother_residence_Mmc = this.draft_details_array.placeofbirth.place_of_birth_mmc;
      this.VillageWard('staticstical');
      this.draft_details_array.legal.mother_residence_Village = this.draft_details_array.placeofbirth.place_of_birth_village;
    } else {
      this.draft_details_array.legal.mother_residence_state = '';
      this.staticstical_district_array = [];
      this.draft_details_array.legal.mother_residence_district = '';
