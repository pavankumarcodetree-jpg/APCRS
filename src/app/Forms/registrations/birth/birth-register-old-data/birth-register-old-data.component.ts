...
@Component({
  selector: 'app-birth-register-old-data',
  templateUrl: './birth-register-old-data.component.html',
  styleUrl: './birth-register-old-data.component.css'
})
export class BirthRegisterOldDataComponent {
  contentuploadurl = '';
  contentshowurl = '';
  ...
  constructor(
    ...
    this.Dateoifreportmindate = this.convertToDate('01-01-1970');
  ...
  draft_details_array = {
    language: {
      ...
      registrationno: '',
      dateofregistration: this.datepipe.transform(new Date(), 'dd-MM-yyyy')
    },
    childinformation: {
      childnotnamed: false,
      dateofbirthmask: '',
  ...
  ngOnInit(): void {
    ...
    console.log(this.brapplicationid);
    try {
      if (sessionStorage.getItem('_Uenc') !== '') {
        ...
        if (this.obj != '' && this.obj != undefined && this.obj != null) {
          ...
          this.draft_details_array.language.dateofreport = this.datepipe.transform(today, 'dd-MM-yyyy');

          // Set Date of Birth maxDate to one day before Date of Registration
          this.dateofbirthmax = new Date(today);
          this.dateofbirthmax.setDate(this.dateofbirthmax.getDate());
  ...
  async getdraftinsert(): Promise<void> {
    try {
      ...
      let rsdata: any = await this.auth.auth_pkgcrsoldbirth_service(req);
      this.spinner.hide();
      if (rsdata.code) {
  ...
  disabable = false; ruralurbandisable: boolean = false;
  async PlaceofBirthchange() {
    ...
    if (this.draft_details_array.placeofbirth.place_of_birth == "1" && this.draft_details_array.placeofbirth.place_of_birth_hospital == "") {
      this.birthinfohide = false;

      this.draft_details_array.information.Informant_fullname = '';
      this.draft_details_array.information.Informant_fullname_t = '';
      this.draft_details_array.information.Informant_fullname_val = '';
      this.draft_details_array.information.Informant_email_id = '';
      this.draft_details_array.information.Informant_mobile_no = '';
      this.draft_details_array.information.Informant_aadhhar_no_mask = '';
      this.draft_details_array.information.Informant_aadhhar_no = '';
      this.draft_details_array.information.Informant_address = '';
      ...
      if (this.photoselectedFiles.length != 0) {
        ...
        if (this.photoselectedFiles.length > 0) {
          ...
          for (let ph = 0; ph < maxlen; ph++) {
            ...
            phform.append('input01', 'OldBirth');
            phform.append('input02', 'oldbirthregistration');
...
debugger
  ...
  //Masters End 
  dateofbirthmax!: Date; dateofbirthmin!: Date; mindateForRegistration!: Date;
  ...
  onDateChange(event: any, type: any) {
    ...
    if (type == 'language') {
      ...
      this.mindateForRegistration = new Date(event);

      const dateOfReporting = new Date(event);

      // Set Date of Birth maxDate to one day before Date of Registration
      this.dateofbirthmax = new Date(dateOfReporting);
      this.dateofbirthmax.setDate(this.dateofbirthmax.getDate());

      this.confirmdateofbirthmin = new Date(dateOfReporting);
      this.confirmdateofbirthmax.setDate(this.confirmdateofbirthmax.getDate());

      // Validate Date of Birth if it already exists
      if (this.draft_details_array.childinformation.dateofbirth) {
        const dateOfBirth = new Date(this.draft_details_array.childinformation.dateofbirth);

        if (dateOfBirth >= dateOfReporting) {
          //alert("Date of Birth must be before Date of Registration.");
          this.draft_details_array.childinformation.dateofbirth = ''; // Reset invalid date 
        }
      }
      if (this.draft_details_array.childinformation.confirmdateofbirth) {
        const ConfimdateOfBirth = new Date(this.draft_details_array.childinformation.confirmdateofbirth);
        if (ConfimdateOfBirth >= dateOfReporting) {
          //alert("Date of Birth must be before Date of Registration.");
          this.draft_details_array.childinformation.confirmdateofbirth = ''; // Reset invalid date
        }
      }

    } if (type == 'dateofregistration') {
      this.draft_details_array.language.dateofregistration = formattedDate || '';

      // const reportDateString = this.draft_details_array?.language?.dateofreport ?? '';
      // const registrationDateString = event ?? '';

      // if (reportDateString && registrationDateString) {
      //   const reportDate = new Date(reportDateString.split('-').reverse().join('-'));
      //   const registrationDate = new Date(registrationDateString.split('-').reverse().join('-'));

      //   if (registrationDate < reportDate) {
      //     //alert('Date of Registration must be equal to or later than Date of Reporting');
      //     this.draft_details_array.language.dateofregistration = '';
      //   }
      // }
    }
    if (type == 'DateofBirth') {
  ...
  @ViewChild('registrationno') registrationno!: NgModel;
  @ViewChild('dateofregistration') dateofregistration!: NgModel;
...
debugger
    ...
    if (this.registrationno) {
      this.registrationno.control.markAsTouched();
      this.registrationno.control.updateValueAndValidity();
      if (this.registrationno.invalid) isInvalid = true;
    }
    if (this.dateofregistration) {
      this.dateofregistration.control.markAsTouched();
      this.dateofregistration.control.updateValueAndValidity();
      if (this.dateofregistration.invalid) isInvalid = true;
    }
    ...
    // }
    if (this.documentlist.length == 0 && !this.fileSelected) {
      this.ngSupportingDocuments.control.markAsTouched();
      this.ngSupportingDocuments.control.updateValueAndValidity();
      if (this.ngSupportingDocuments.invalid) isInvalid = true;
    }
    if (this.documentlist.length == 0 && !this.multifileSelected) {
  ...
  birthinformationdetails() {
    ...
    this.birthinfohide = false;
    if (this.draft_details_array.placeofbirth.place_of_birth == "1") {
      let HospitalDetails = this.Hospital_master_array.find(e => e.HOSPITAL_CODE == this.draft_details_array.placeofbirth.place_of_birth_hospital);
  ...
  deletePreview(index: number): void {
    ...
    if(this.photoselectedFiles.length>0){
      ...
      this.multiphotoselectedFiles.splice(index, 1);
    }
    if (this.documentlist.length > 0 && this.fileSelected) {
      this.documentdelte(this.documentlist[index]);
    }
  }
}
